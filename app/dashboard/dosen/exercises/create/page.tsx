...
export default function CreateDosenExercisePage() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!res.ok) {
        throw new Error(result.message || "Gagal membuat latihan");
      }

      alert("Exercise berhasil dibuat.");
      router.push("/dashboard/dosen/exercises");
    } catch (error) {
      console.error("CREATE_EXERCISE_ERROR:", error);
      alert("Gagal membuat exercise.");
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="dashboard-card neu-card" style={{ display: "grid", gap: 16 }}>
        ...
        {questionType === "multiple_choice" && (
          <div className="dashboard-card neu-card" style={{ display: "grid", gap: 12 }}>
            ...
            {options.map((item, index) => (
              <div key={item.label} className="auth-field">
        ...
        {questionType === "essay" && (
          <div className="auth-field">
            ...
            <textarea
              ...
              value={correctAnswer}
              onChange={(e) => setCorrectAnswer(e.target.value)}
              placeholder="Isi jawaban referensi / keyword utama"
              rows={4}
            />
          </div>
        )}
        ...
        <div className="auth-field">
          ...
          <textarea
            ...
            placeholder="Penjelasan jika jawaban benar atau pembahasan singkat"
            rows={3}
          />
        </div>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
          ...
          <button className="neu-button" type="button" onClick={handleSubmit} disabled={saving}>
            {saving ? "Menyimpan..." : "Simpan Exercise"}
          </button>
        </div>
      </div>
    </div>
